# Changements - Configuration du Projet

## Décembre 2025 - Réorganisation de la structure du projet

### Changements structurels

- **Suppression du projet CSharp** : Le dossier `JTrading.NewsManager.CSharp/` a été entièrement supprimé
- **Migration Python vers la racine** : Tous les fichiers Python ont été déplacés de `python/` vers la racine du projet
- **Mise à jour de la configuration VSCode** : Les chemins dans `.vscode/launch.json` ont été mis à jour pour refléter la nouvelle structure
- **Fusion des configurations Claude** : Les fichiers `.claude/settings.local.json` ont été fusionnés

### Structure avant
```
MyScrapers/
├── JTrading.NewsManager.CSharp/    # Projet C# (supprimé)
└── python/                          # Projet Python (déplacé)
```

### Structure après
```
MyScrapers/                          # Projet Python à la racine
├── app.py
